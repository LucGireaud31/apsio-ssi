...
export function DropDown(props: DropDownProps) {
  ...
  function handleSubmit(accessor: string, newValue: string) {
    setProfil(accessor, newValue, clearPassword);
    setItems(
      items.map((item) => {
    ...
    <View>
      ...
      >
        <View style={styles.header}>
          ...
          <Image
            source={require("../../../assets/icons/caret-right.png")}
            style={dynamicStyle.caretIcon}
          />
        </View>
        {!isOpen && (
          <View style={styles.content}>
            {filteredItems.length == 0 && (
              <Text style={styles.noData}>Aucune données</Text>
            )}
        ...
        <View style={styles.content}>
          {items.map((item, i) => (
            <Input
              ...
              onSubmit={(value) => handleSubmit(item.accessor, value)}
            />
          ))}
        </View>
      )}
